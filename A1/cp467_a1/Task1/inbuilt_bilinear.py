...
#in-built function for bilinear interpolation
lena_bilinear_cv_down = cv2.resize(lena_img, (new_width, new_height),fx=scale_x, fy=scale_y ,interpolation=cv2.INTER_LINEAR)

#displaying the scaled down image
cv2.imshow("Scaled down Image", lena_bilinear_cv_down)
cv2.waitKey(0)
cv2.destroyAllWindows()

#save the scaled down image
cv2.imwrite("lena_bilinear_cv_down.tif", lena_bilinear_cv_down)
...
lena_bilinear_cv = cv2.resize(lena_bilinear_cv_down, (scaled_up_width, scaled_up_height),fx=scale_up_x, fy=scale_up_y ,interpolation=cv2.INTER_LINEAR)

#displaying the scaled up image
cv2.imshow("Scaled Up Image", lena_bilinear_cv)
cv2.waitKey(0)
cv2.destroyAllWindows()

#save the scaled down image
cv2.imwrite("lena_bilinear_cv.tif", lena_bilinear_cv)
